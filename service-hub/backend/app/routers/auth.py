...
from app.schemas import OtpRequestIn, OtpVerifyIn, RefreshIn, TokenOut, UserOut, UserLocationUpdate
...
async def update_my_location(
    payload: UserLocationUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user),
):
    """Manual, OLX-style location — no maps required. Any logged-in user can set it."""
    user.location_text = payload.location_text
    db.commit()
    db.refresh(user)
    return user
