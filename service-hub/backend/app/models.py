...
class User(Base):
    ...
    location_text = Column(String, nullable=True)  # e.g. "Bhelma, Anuppur, MP" — set manually, no maps needed
...
class JobCard(Base):
    """
    Digital proof-of-work, filled in by the provider once a job is
    COMPLETED. No photo fields yet — this app has no file-storage
    integration wired up, and a "before/after photos" field with nothing
    behind it would be exactly the fake functionality the brief warns
    against. Add photos once Supabase Storage (or similar) is connected.
    """
    ...
    def warranty_active(self) -> bool:
        ...
        expires = self.created_at + timedelta(days=self.warranty_days)
        now = utcnow()
...
class WarrantyClaim(Base):
    ...
    status = Column(SAEnum(WarrantyClaimStatus), nullable=False, default=WarrantyClaimStatus.OPEN, index=True)
