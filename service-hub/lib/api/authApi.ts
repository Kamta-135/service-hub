...
export function guestLogin(role: UserRole) {
  return apiFetch<TokenResponse>(`/auth/guest?role=${role}`, { method: "POST" });
}

export function updateMyLocation(token: string, locationText: string) {
  return apiFetch(`/auth/me/location`, { method: "PATCH", token, body: { location_text: locationText } });
}
