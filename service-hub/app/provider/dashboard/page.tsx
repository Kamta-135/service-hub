...
import { updateMyProviderProfile } from "@/lib/api/providerApi";
import { createJobCard } from "@/lib/api/jobCardApi";
...
export default function ProviderDashboardPage() {
  ...
  const [quoteSubmitted, setQuoteSubmitted] = useState<Set<string>>(new Set());
  const [locationStatus, setLocationStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");
  const [chatOpenId, setChatOpenId] = useState<string | null>(null);
  const [jobCardFormId, setJobCardFormId] = useState<string | null>(null);
  const [jobCardWork, setJobCardWork] = useState("");
  const [jobCardAmount, setJobCardAmount] = useState("");
  const [jobCardWarranty, setJobCardWarranty] = useState("");
  const [jobCardSubmitted, setJobCardSubmitted] = useState<Set<string>>(new Set());
  ...
  async function handleSetLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          await updateMyProviderProfile(accessToken, {
  ...
  async function handleSubmitQuote(id: string) {
    if (!accessToken) return;
    ...
    setBusyId(id);
    try {
      await submitQuote(accessToken, id, {
        amount,
        eta_minutes: quoteEta ? parseInt(quoteEta, 10) : undefined,
      });
      setQuoteSubmitted((prev) => new Set(prev).add(id));
      setQuoteFormId(null);
      setQuoteAmount("");
      setQuoteEta("");
    } catch {
      setError("Couldn't submit your quote — try again.");
    } finally {
      setBusyId(null);
    }
  }

  async function handleCreateJobCard(id: string) {
    if (!accessToken || !jobCardWork.trim()) {
      setError("Describe the work performed.");
      return;
    }
    setBusyId(id);
    try {
      await createJobCard(accessToken, id, {
        work_performed: jobCardWork.trim(),
        final_amount: jobCardAmount ? parseFloat(jobCardAmount) : undefined,
        warranty_days: jobCardWarranty ? parseInt(jobCardWarranty, 10) : undefined,
      });
      setJobCardSubmitted((prev) => new Set(prev).add(id));
      setJobCardFormId(null);
      setJobCardWork("");
      setJobCardAmount("");
      setJobCardWarranty("");
    } catch {
      setError("Couldn't save the job card — try again.");
    } finally {
      setBusyId(null);
    }
  }
      ...
      <aside className="sticky top-0 z-10 hidden h-screen w-64 shrink-0 flex-col border-r border-black/5 bg-white px-5 py-6 md:flex">
        ...
        <nav className="mt-6 flex flex-1 flex-col gap-1">
          ...
          <button
            ...
            className="flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-left text-sm font-semibold text-black/60 hover:bg-black/[0.03] hover:text-ink disabled:opacity-50"
          >
            📍 {locationStatus === "saving" ? "Getting location..." : locationStatus === "saved" ? "Location saved ✓" : "Set my location"}
          </button>
          {locationStatus === "error" && (
            <p className="px-3.5 text-xs text-red-500">Couldn't get your location — check browser permissions.</p>
          )}
          <button
            onClick={() => {
              logout();
              ...
              <div className="mt-3 space-y-3">
                {newRequests.map((r) => (
                  <div key={r.id} className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm">
                    ...
                    {quoteSubmitted.has(r.id) ? (
                      <p className="mt-2 text-center text-xs font-medium text-black/40">Quote sent — waiting on the customer.</p>
                    ) : quoteFormId === r.id ? (
                      <div className="mt-2 rounded-xl border border-black/10 bg-canvas p-3">
                        <div className="flex gap-2">
                          <input
                            ...
                            className="h-9 w-1/2 rounded-lg border border-black/10 bg-white px-2 text-sm"
                          />
                          <input
                            type="number"
                            placeholder="ETA (min)"
                        ...
                        <div className="mt-2 flex gap-2">
                          <button
                            ...
                            disabled={busyId === r.id}
                            className="h-8 flex-1 rounded-lg bg-ink text-xs font-bold text-white disabled:opacity-40"
                          >
                            Send quote
                          </button>
                          <button onClick={() => setQuoteFormId(null)} className="h-8 px-3 text-xs font-semibold text-black/50">
                            Cancel
                          </button>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={() => setQuoteFormId(r.id)}
                        className="mt-2 w-full text-center text-xs font-semibold text-brand underline"
                      >
                        Or send a price quote instead
                      </button>
                    )}
                  </div>
                ))}
                ...
                {myRequests.map((r) => {
                  ...
                  return (
                    <div key={r.id} className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm">
                      ...
                      {r.status === "completed" && (
                        jobCardSubmitted.has(r.id) ? (
                          <p className="mt-2 text-center text-xs font-medium text-green-700">✓ Job card submitted</p>
                        ) : jobCardFormId === r.id ? (
                          <div className="mt-2 rounded-xl border border-black/10 bg-canvas p-3">
                            <textarea
                              value={jobCardWork}
                              onChange={(e) => setJobCardWork(e.target.value)}
                              rows={2}
                            ...
                            <div className="mt-2 flex gap-2">
                              <input
                                ...
                                value={jobCardAmount}
                                onChange={(e) => setJobCardAmount(e.target.value)}
                                className="h-9 w-1/2 rounded-lg border border-black/10 bg-white px-2 text-sm"
                              />
                              <input
                                type="number"
                                placeholder="Warranty (days)"
                                value={jobCardWarranty}
                                onChange={(e) => setJobCardWarranty(e.target.value)}
                                className="h-9 w-1/2 rounded-lg border border-black/10 bg-white px-2 text-sm"
                              />
                            </div>
                            <div className="mt-2 flex gap-2">
                              <button
                                onClick={() => handleCreateJobCard(r.id)}
                                disabled={busyId === r.id}
                                className="h-8 flex-1 rounded-lg bg-ink text-xs font-bold text-white disabled:opacity-40"
                              >
                                Save job card
                              </button>
                              <button onClick={() => setJobCardFormId(null)} className="h-8 px-3 text-xs font-semibold text-black/50">
                                Cancel
                              </button>
                            </div>
                          </div>
                          ...
                          <button
                            onClick={() => setJobCardFormId(r.id)}
                            className="mt-2 w-full text-center text-xs font-semibold text-ink underline"
                          >
                            📋 Fill out job card
                          </button>
                        )
                      ...
                      {chatOpenId === r.id ? (
                        accessToken && user && <ChatBox requestId={r.id} accessToken={accessToken} myUserId={user.id} />
                      ) : (
                        <button
                          onClick={() => setChatOpenId(r.id)}
                          className="mt-2 w-full text-center text-xs font-semibold text-brand underline"
                        >
                          💬 Open chat
                        </button>
